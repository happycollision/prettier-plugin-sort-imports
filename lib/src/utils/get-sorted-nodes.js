...
var getSortedNodes = function (nodes, options) {
    ...
    var importOrder = options.importOrder, importOrderSeparationGroups = options.importOrderSeparationGroups;
    ...
    if (importOrderSeparationGroups.length === 0) {
        importOrderSeparationGroups = __spreadArray([], importOrder, true);
    }
    if (!importOrder.includes(constants_1.THIRD_PARTY_MODULES_SPECIAL_WORD)) {
        ...
        importOrderSeparationGroups = __spreadArray([
            constants_1.THIRD_PARTY_MODULES_SPECIAL_WORD
        ], importOrderSeparationGroups, true);
    }
    ...
    for (var _i = 0, originalNodes_1 = originalNodes; _i < originalNodes_1.length; _i++) {
        ...
        var matchedGroup = (0, get_import_nodes_matched_group_1.getImportNodesMatchedGroup)(node, importOrder);
        importOrderGroups[matchedGroup].push(node);
    }
    var latestMatchedGroup = '';
    for (var _a = 0, _b = (0, lodash_1.zip)(importOrder, importOrderSeparationGroups); _a < _b.length; _a++) {
        var _c = _b[_a], group = _c[0], thisGroupName = _c[1];
        if (!group)
            continue;
        ...
        if (latestMatchedGroup === '') {
            latestMatchedGroup = thisGroupName;
        }
